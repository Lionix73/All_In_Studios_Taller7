    ...
    public class RebindActionUI : MonoBehaviour
    {
        ...
        private void PerformInteractiveRebind(InputAction action, int bindingIndex, bool allCompositeParts = false)
        {
            ...
            m_RebindOperation = action.PerformInteractiveRebinding(bindingIndex)
                .WithControlsExcluding("<Mouse>") // Exclude mouse by default.
            ...
            if (m_RebindText != null)
            {
                var text = !string.IsNullOrEmpty(m_RebindOperation.expectedControlType)
                    ? $"{partName}Waiting for {m_RebindOperation.action.name} input..."
        ...
        private bool CheckForDuplicateBindings(InputAction action, int bindingIndex, bool allCompositeParts = false)
        {
            ...
            foreach (InputBinding binding in action.actionMap.bindings)
            {
                ...
                {
                    Debug.LogWarning($"Binding '{newBinding.effectivePath}' for action '{newBinding.action}' would be a duplicate of binding for action '{binding.action}'");
                    return true;
                }
            ...
            if (allCompositeParts)
            {
                for (int i = 1; i < bindingIndex; i++)
                {
                    ...
                    {
                        Debug.LogWarning($"Binding '{newBinding.effectivePath}' for action '{newBinding.action}' would be a duplicate of binding for action '{action.bindings[i].action}'");
                        return true;
                    }
                }
            }
