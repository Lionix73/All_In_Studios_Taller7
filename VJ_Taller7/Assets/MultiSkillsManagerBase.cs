...
public class MultiSkillsManagerBase : NetworkBehaviour
{
    ...
    [Rpc(SendTo.Everyone)]
    public void DecreasePassiveSkillMaskRpc(float cooldown)
    {
        if (!IsOwner) return;

        StartCoroutine(DecreasePassiveSkillMask(cooldown));
    }
