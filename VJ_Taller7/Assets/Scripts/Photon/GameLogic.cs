...
public class GameLogic : NetworkBehaviour, IPlayerJoined, IPlayerLeft
{
    ...
    [Networked] private PlayerControllerPhoton Winner { get; set; }
    [Networked, OnChangedRender(nameof(GameStateChanged))] private GameState State { get; set; }
    [Networked, Capacity(4)] private NetworkDictionary<PlayerRef, PlayerControllerPhoton> Players => default;
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (Runner.IsServer && Winner == null && other.attachedRigidbody != null && other.attachedRigidbody.TryGetComponent(out PlayerControllerPhoton player))
        {
            UnreadyAll();
            //Winner = player;
            State = GameState.Waiting;
    ...
    public override void FixedUpdateNetwork()
    {
        ...
        if (Runner.IsServer && State == GameState.Waiting)
        {
            ...
            foreach (KeyValuePair<PlayerRef, PlayerControllerPhoton> player in Players)
            {
    ...
    public void PlayerJoined(PlayerRef player)
    {
        if (HasStateAuthority)
        {
            ...
            NetworkObject playerObject = Runner.Spawn(playerPrefab, Vector3.up, Quaternion.identity, player); //Spawn Method to instatiate player prefab, spawn player on host and replicate the spawn to other clients
            Players.Add(player, playerObject.GetComponent<PlayerControllerPhoton>()); //Add a reference to the newly spawned player script to our dictionary
    ...
    public void PlayerLeft(PlayerRef player)
    {
        ...
        if (Players.TryGet(player, out PlayerControllerPhoton playerBehaviour))
        {
