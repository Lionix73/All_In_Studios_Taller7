...
public class EnemyMovementMulti : NetworkBehaviour
{
    ...
    {
        if(IsServer) networkState.Value = defaultState;       
    }
    ...
    private void Update()
    {
        if (IsServer && Vector3.Distance(transform.position, lastSyncedPos) > 0.5f)
        {
            lastSyncedPos = transform.position;
            SyncPositionClientRpc(lastSyncedPos);
        }
    ...
    public void ResumeMovement(){
        if (agent != null && agent.enabled)
        {
            ...
            networkState.Value = EnemyState.Chase;
        }
    }
